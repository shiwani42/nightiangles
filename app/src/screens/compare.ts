...
export function renderCompare(root: HTMLElement) {
  root.innerHTML = `
    <header>
      <h1>Price Decoder</h1>
      <p class="tag">Scan two products. We'll show you where the price gap comes from.</p>
    </header>
    <main class="screen-compare">
      <div id="status" class="status">tap "Scan slot A" to start</div>
  ...
  function renderDiff() {
    ...
    if (products.A.product_code === products.B.product_code) {
      diffEl.innerHTML = `<div class="status">Same product in both slots — scan two different items to compare.</div>`;
      return;
    }
    ...
    if (d.gap === 0) {
      diffEl.innerHTML = `
        <div class="diff-card">
          <h3>Same price — but different gear</h3>
          <p>${escapeHTML(d.pricier.name)} and ${escapeHTML(d.cheaper.name)} both at CHF ${d.pricier.price_chf.toFixed(0)}.</p>
    ...
    diffEl.innerHTML = `
      <div class="diff-card">
        <h3><strong>${escapeHTML(d.pricier.name)}</strong> is <strong>CHF ${d.gap.toFixed(0)}</strong> more than ${escapeHTML(d.cheaper.name)}</h3>
        <p class="diff-card__lead">Where the money goes:</p>
        <ul class="diff-list">
          ${d.reasons
            .map(
              (r) => `<li>
  ...
  async function initScanner() {
    ...
    barcodeCapture.addListener({
      didScan: async (_mode, session) => {
        ...
        if (!product) {
          setStatus(`Code "${code}" is not in our catalog — try another.`);
          return;
        }
  ...
  async function startScanning(slot: Slot) {
    ...
    setStatus(`Scan a barcode for slot ${slot}`);
  }
