...
export function renderSmoke(root: HTMLElement) {
  root.innerHTML = `
    <header>
      ...
      <p class="tag">Step-by-step camera test. Tells us exactly which step fails.</p>
    </header>
    <main>
      <button id="start" class="primary">Run camera test</button>
      <div id="status" class="status">press the button</div>
