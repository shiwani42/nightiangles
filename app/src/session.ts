...
function client(): SupabaseClient {
  if (!supabaseConfigured) {
    throw new Error(
      "Supabase not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Render.",
    );
  }
  if (!_client) {
...
const STORAGE = "nightiangles.session";
...
export class Session {
  ...
  async connect(): Promise<void> {
    const c = client();
    const channel = c.channel(`nightiangles:${this.code}`, {
      config: {
