...
- **2026-06-19 (v1 complete + deployed)** — Phases 1-3 done and live on Render.
    ...
    - **Render deploy**: live at `https://nightingale-hack.onrender.com`, configured via `render.yaml` blueprint (static site, root `app/`, `npm install && npm run build`, publish `dist/`, SPA rewrite, long cache on `/assets/*`). Env var `VITE_SCANDIT_LICENSE_KEY` set via the Render UI. Bundle ID for the Scandit license matches the deploy hostname.
